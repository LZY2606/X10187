...
class _PicklerSession:
    """Mutable state for a single top-level encode operation.

    A long-lived ``Pickler`` only carries read-only configuration
    (backend, handler registry, flags) between runs.  Everything that is
    accumulated while walking one object graph -- reference IDs, recursion
    depth, the memo caches and the GC anchors -- lives on a session object
    so that a finished or failed run never leaves per-run state behind on
    the pickler itself.

    The session holds references to the read-only configuration it relies
    on (the JSON backend and the handler registry) but deliberately does
    not reference the pickler itself: a session->pickler edge would create
    a reference cycle and keep ephemeral picklers (and the objects they
    anchored) alive until the cyclic GC runs.
    """

    __slots__ = ("backend", "handlers", "depth", "objs", "seen", "flattened")

    def __init__(self, context: "Pickler") -> None:
        # Read-only configuration: JSON backend and handler registry.
...
class Pickler:
    ...
    # that introspect the pickler (e.g. the numpy extension handler).
    @property
    def _depth(self) -> int:
    ...
    def reset(self) -> None:
        """Discard all per-run state by starting a fresh session."""
        self._session = _PicklerSession(self)
    ...
    def flatten(self, obj: Any, reset: bool = True) -> Any:
        ...
        if reset:
            self.reset()
            try:
                return self._flatten_entry(obj)
            finally:
                # A top-level flatten must not leave per-run state behind,
                # even when a handler or __getstate__ raises.  Recursive
                # calls (reset=False) join the current session instead and
                # are cleaned up by the outermost call.
                self.reset()
        return self._flatten_entry(obj)

    def _flatten_entry(self, obj: Any) -> Any:
